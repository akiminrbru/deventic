  ...
  if (header) {
    ...
    window.addEventListener('scroll', function () {
      if (window.pageYOffset > sticky) {
        header.classList.add("header--scroll"); //main.style.paddingTop = sticky + 'px';
      } else {
...
var menuOpener = function menuOpener() {
  var menuToggler = document.querySelector('[data-action="menu-open"]');
  ...
  var subMenuHandler = function subMenuHandler(evt) {
    evt.preventDefault(); // if (activeSubMenu) closeSubMenu();
  ...
  var outOfAreaHandler = function outOfAreaHandler(evt) {
...
var CreateNewElement = /*#__PURE__*/(/* unused pure expression or super */ null && (function () {
...
var checkField = function checkField(fields) {
  ...
  fields.forEach(function (field) {
    ...
    if (Boolean(checkType(newField))) {
      ...
      var _mistake = checkType(newField) || 'Заполните поле';

      if (!field.closest('label').querySelector('.form__mistake')) {
        var _mistakeBlock = document.createElement('span');

        _mistakeBlock.classList.add('form__mistake');

        field.closest('label').append(_mistakeBlock);
      }

      field.closest('label').querySelector('.form__mistake').innerText = _mistake;
      mistakes++;
    }
  });
  if (fields[0].closest('form').querySelector(".cf7sr-g-recaptcha").closest('label').classList.contains('mistake')) fields[0].closest('form').querySelector(".cf7sr-g-recaptcha").closest('label').classList.remove('mistake');
...
;// CONCATENATED MODULE: ./app/js/components/animation.js
var animateElts = function animateElts() {
  var timeInd = 0;
  var imageBlocks = document.querySelectorAll('.animate-block');
  var itemsStart = document.querySelectorAll('.animate-elt');

  if (itemsStart.length > 0) {
    itemsStart.forEach(function (item) {
      return item.classList.add('hovered');
    });
  }
  ...
  if (imageBlocks.length > 0) {
    var observer = new IntersectionObserver(function (entries, observer) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          var items = entry.target.querySelectorAll('.animate-elt');

          if (items.length > 0) {
            items.forEach(function (item, i) {
              item.classList.remove('hovered');
              if (timeInd !== 0) item.style.transitionDelay = "".concat(timeInd * i, "s");
            });
          }
    ...
    imageBlocks.forEach(function (item) {
      observer.observe(item);
    });
  }
};
;// CONCATENATED MODULE: ./app/js/components/tabs.js
var tabsOpener = function tabsOpener(className, bodyClassName) {
  ...
  var openHandler = function openHandler(evt) {
    evt.preventDefault();
    var target = evt.target.closest(className);
    if (!target) return;
    var togglerBody = Boolean(target.closest(bodyClassName)) ? target.closest(bodyClassName) : target.closest(className).parentElement;
  ...
  var main = togglers[0].closest('section') || togglers[0].closest('main');
...
var tabsOpenerOutOfArea = function tabsOpenerOutOfArea(className, elementClassName) {
  var togglers = document.querySelectorAll(className);
  ...
  var clickCloseHandler = function clickCloseHandler(evt) {
    if (parent.contains(evt.target)) return;
    closeHandler();
  };

  var keyCloseHandler = function keyCloseHandler(evt) {
    if (evt.code === 'Escape') {
      closeHandler();
    }
  };
...
core.use([Navigation, Pagination, Autoplay, EffectFade]); // Если будет не нужна - убрать
...
var advsSwiper = new core('.advs-slider', advsSliderSettings);
var fbSliderSettings = {
  slidesPerView: 1,
  spaceBetween: 0,
  loop: false,
  observer: true,
...
var fbSwiper = new core('.fb-slider', fbSliderSettings);

function casesSwiperInit() {
  if (window.matchMedia("(min-width: 540px)").matches) {
    var casesSliderSettings = {
      slidesPerView: 'auto',
      spaceBetween: 16,
      breakpoints: {
        540: {
    ...
    var casesSwiper = new core('.cases-block__slider', casesSliderSettings);
  }

  if (window.matchMedia('(min-width:540px) and (max-width: 999px)').matches) {
    var _casesSliderSettings = {
      slidesPerView: 'auto',
      spaceBetween: 24,
      loop: false,
      observer: true,
    ...
    var articlesSwiper = new core('.blog-section__slider', _casesSliderSettings);

    var _casesSwiper = new core('.cases-block__slider-vert', _casesSliderSettings);
  }
}
...
var valuesSwiper = new core('.hh-values-slider__inside', valuesSliderSettings); // обработка popup 

try {
  var appointmentOpeners = document.querySelectorAll('[data-action="order"]');
  var popupAppointmentElt = document.querySelector('[data-popup="order"]');
  var js_forms = document.querySelectorAll('form');
